...
module.exports = function fetchTemplate(templatesPath, baseTemplateFn) {
    return (request, parseTemplate, getServiceUrl) => {
        //const pathname = url.parse(request.url, true).pathname;
        //const templatePath = path.join(templatesPath, pathname) + '.html';
        const templatePath = path.join(templatesPath) + '.html';
        return readFile(templatePath)
            .then(baseTemplate => {
                if (typeof baseTemplateFn !== 'function') {
                    return parseTemplate(baseTemplate,null, request, getServiceUrl);
                }
                ...
                return readFile(baseTemplatePath)
                    .then(baseTemplate => parseTemplate(baseTemplate, pageTemplate, request, getServiceUrl));
            });
    };
};
